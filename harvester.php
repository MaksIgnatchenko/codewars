...
function harvester_rescue(array $data): string {
    $xH = $data['harvester'][0];
    $yH = $data['harvester'][1];
    $xW = $data['worm'][0][0];
    $yW = $data['worm'][0][1];
    $xC = $data['carryall'][0][0];
    $yC = $data['carryall'][0][1];
    $speedW = $data['worm'][1];
    $speedC = $data['carryall'][1];
    $distanceHW = sqrt(pow(abs($xH-$xW), 2) + pow(abs($yH-$yW), 2));
    $distanceHC = sqrt(pow(abs($xH-$xC), 2) + pow(abs($yH-$yC), 2));
    $timeW = $distanceHW / $speedW;
    $timeC = ($distanceHC / $speedC) + 1;
    if ($timeC < $timeW) return "The spice must flow! Rescue the harvester!";
